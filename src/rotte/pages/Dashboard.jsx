import {
  ShoppingCart, Truck, Ban, DollarSign, Users, Star,
...
// Sample data (ganti dengan import sesuai struktur kamu)
const customers = [
  { id: 1, name: "Aulia Syafitri", loyalty: "Gold", totalSpent: 2500000, points: 1250 },
  { id: 2, name: "Fitriana Tasya", loyalty: "Silver", totalSpent: 1500000, points: 750 },
  { id: 3, name: "Givo Fadillah", loyalty: "Gold", totalSpent: 4200000, points: 2100 },
  { id: 4, name: "Budi Santoso", loyalty: "Bronze", totalSpent: 240000, points: 120 },
  { id: 5, name: "Siti Aminah", loyalty: "Silver", totalSpent: 960000, points: 480 },
  { id: 10, name: "Rudi Hartono", loyalty: "Gold", totalSpent: 6000000, points: 3000 },
  { id: 24, name: "Bayu Saputra", loyalty: "Gold", totalSpent: 5500000, points: 2750 },
  { id: 27, name: "Elsa Maharani", loyalty: "Gold", totalSpent: 6200000, points: 3100 },
];

const orders = [
  { id: "ORD001", customerId: 1, customerName: "Aulia Syafitri", items: ["Roti Coklat"], totalPrice: 45000, status: "Completed", orderDate: "2025-04-28" },
  { id: "ORD002", customerId: 2, customerName: "Fitriana Tasya", items: ["Donat Gula"], totalPrice: 32000, status: "Completed", orderDate: "2025-04-27" },
  { id: "ORD003", customerId: 3, customerName: "Givo Fadillah", items: ["Roti Gandum"], totalPrice: 68000, status: "Pending", orderDate: "2025-04-29" },
  { id: "ORD004", customerId: 4, customerName: "Budi Santoso", items: ["Roti Tawar"], totalPrice: 28000, status: "Completed", orderDate: "2025-04-26" },
  { id: "ORD005", customerId: 5, customerName: "Siti Aminah", items: ["Croissant"], totalPrice: 55000, status: "Cancelled", orderDate: "2025-04-25" },
  { id: "ORD006", customerId: 6, customerName: "Rizki Ramadhan", items: ["Roti Abon"], totalPrice: 23000, status: "Completed", orderDate: "2025-04-24" },
  { id: "ORD007", customerId: 7, customerName: "Dewi Lestari", items: ["Roti Sosis"], totalPrice: 47000, status: "Completed", orderDate: "2025-04-23" },
];
...
// ─── Stat Card ───────────────────────────────────────────────────────────────
function StatCard({ icon: Icon, label, value, sub, trend, sparkData, accent, onClick }) {
    const isUp = trend >= 0;
    return (
        <div
            onClick={onClick}
...
// ─── Order Status Pie Chart via Canvas ───────────────────────────────────────
...
export default function Dashboard() {
    ...
    return (
        <div style={{ background: "#F7F7F7", minHeight: "100vh", padding: "24px 32px 32px 32px" }}>
            {/* Page Header */}
            <div style={{ marginBottom: 28 }}>
                <div style={{ fontSize: 12, color: "#999", marginBottom: 6 }}>Home</div>
                <h1 style={{ fontSize: 28, fontWeight: 700, color: "#1A1A1A", margin: 0, marginBottom: 4 }}>Dashboard</h1>
                <p style={{ fontSize: 13, color: "#999", margin: 0 }}>Ringkasan bisnis Rotte Bakery</p>
            </div>

            {/* ── Stat Cards ── */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 24 }}>
                <StatCard icon={ShoppingCart} label="Total Orders" value={totalOrders} sub="Semua waktu" trend={12} sparkData={sparkOrders} onClick={() => setModal({ title: "Total Orders", value: totalOrders })} />
                <StatCard icon={Truck} label="Selesai" value={totalCompleted} sub="Completed orders" trend={8} sparkData={[18, 22, 20, 26, 23, totalCompleted]} onClick={() => setModal({ title: "Completed Orders", value: totalCompleted })} />
                <StatCard icon={Ban} label="Dibatalkan" value={totalCancelled} sub="Cancelled orders" trend={-3} sparkData={[5, 3, 6, 4, 5, totalCancelled]} accent="#E24B4A" onClick={() => setModal({ title: "Cancelled", value: totalCancelled })} />
                <StatCard icon={DollarSign} label="Pendapatan" value={`Rp ${(totalRevenue / 1000000).toFixed(1)}jt`} sub="Total revenue" trend={15} sparkData={sparkRevenue} onClick={() => setModal({ title: "Total Revenue", value: `Rp ${totalRevenue.toLocaleString()}` })} />
                <StatCard icon={Users} label="Pelanggan" value={totalCustomers} sub="Terdaftar" trend={5} sparkData={[80, 92, 95, 101, 105, totalCustomers]} onClick={() => setModal({ title: "Total Customers", value: totalCustomers })} />
                <StatCard icon={Star} label="Active Members" value={activeMembers} sub="Loyalty aktif" trend={9} sparkData={[30, 35, 32, 38, 36, activeMembers]} onClick={() => setModal({ title: "Active Members", value: activeMembers })} />
            </div>
                ...
                >
                    ...
                    >
                        <div style={{ width: 48, height: 48, borderRadius: 12, background: MAROON_MUTED, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
                            <ShoppingCart size={24} color={MAROON} />
                        </div>
